#!/usr/bin/env python3
"""
Build Quail Hatch Video v17 - FINAL EDITS v2
- Remove seconds 21-28 section
- Updated humidity text with emoji
- Title fade effects
"""
...
OUTPUT = f"{INPUT_DIR}/QuailHatchVideo_v17.mp4"
AUDIO = f"{INPUT_DIR}/ukulele_source.m4a"
WORKSPACE = "/Users/mirzaie/.openclaw/workspace/v17_build"
...
def create_title_card(text, duration=3):
    """Create title card with fade effect"""
    ...
    for line in lines:
        ...
        for offset in range(5, 0, -1):
            alpha = int(255 * (0.3 / offset))
            draw.text((x, current_y), line, font=title_font, 
...
def build_v17():
    print("Building Quail Hatch Video v17 (FINAL)...")
    ...
    hum_clip = extract_clip(f"{INPUT_DIR}/IMG_0123.MOV", 14, 5)  # Shortened by 1s
    ...
    # Scene 4: Day 17 - SHORTENED to avoid seconds 21-28 overlap
    print("Scene 4: Day 17 flashback")
    egg_clip = extract_clip(f"{INPUT_DIR}/IMG_6093.MOV", 10, 3)  # Shortened
    ...
    # Scene 5: The struggle - START LATER to skip seconds 21-28
    print("Scene 5: The struggle")
    effort_clip = extract_clip(f"{INPUT_DIR}/IMG_7081.MOV", 35, 15, slow_factor=0.5)  # Start later
    ...
    # Scene 8: First hatchlings (moved after main hatch)
    ...
    # Scene 9: Humidity tip - UPDATED TEXT
    print("Scene 9: Humidity tip - Updated")

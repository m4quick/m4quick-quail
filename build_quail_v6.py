...
Combines Option A (story arc) with Option B (quick, satisfying pace)
"""
...
def create_text_clip(text, duration, position="center"):
    ...
    temp_path = f"{WORKSPACE}/temp_text_{hash(text)}.png"
    img.save(temp_path)
    clip = ImageClip(temp_path, duration=duration)
    return clip

def process_video_for_vertical(video_path, start_time, duration, crop_focus="center"):
    ...
    if w > h:  # Horizontal video
        # Scale to target height, crop sides
    ...
    else:  # Already vertical or square
        if w < TARGET_W:  # Need to scale up
            scale_factor = TARGET_W / w
            new_h = int(h * scale_factor)
            clip = clip.resize(width=TARGET_W)
            # Center crop vertically
            crop_y = (new_h - TARGET_H) // 2
            clip = clip.crop(x1=0, y1=crop_y, x2=TARGET_W, y2=crop_y + TARGET_H)
        else:
            # Just resize to fit
...
# Scene 1: "The Wait" - Eggs ready to hatch
print("Scene 1: The Wait (Eggs in incubator)")
clip1 = process_video_for_vertical(f"{INPUT_DIR}/IMG_6093.MOV", 10, 8)
text1 = create_text_clip("Day 17...", 2, "top")
clips.append(CompositeVideoClip([clip1, text1]))

# Scene 2: "The First Sign" - Pipping
print("Scene 2: The First Sign (Pipping)")
clip2 = process_video_for_vertical(f"{INPUT_DIR}/IMG_6138.MOV", 5, 6)
text2 = create_text_clip("It's starting!", 2, "top")
clips.append(CompositeVideoClip([clip2, text2]))

# Scene 3: "The Struggle" - Working to hatch
print("Scene 3: The Struggle")
clip3 = process_video_for_vertical(f"{INPUT_DIR}/IMG_7081.MOV", 30, 10)
text3 = create_text_clip("So much effort...", 2, "top")
clips.append(CompositeVideoClip([clip3, text3]))

# Scene 4: "The Breakthrough" - First chick out
print("Scene 4: The Breakthrough")
clip4 = process_video_for_vertical(f"{INPUT_DIR}/IMG_7081.MOV", 60, 8)
text4 = create_text_clip("Welcome!", 2, "top")
clips.append(CompositeVideoClip([clip4, text4]))

# Scene 5: "Wet and New" - Newly hatched
print("Scene 5: Wet and New")
clip5 = process_video_for_vertical(f"{INPUT_DIR}/IMG_7082.MOV", 2, 6)
text5 = create_text_clip("Just born", 2, "top")
clips.append(CompositeVideoClip([clip5, text5]))

# Scene 6: "The Brooder" - Fluffy chicks
print("Scene 6: The Brooder (Dry fluffy chicks)")
clip6 = process_video_for_vertical(f"{INPUT_DIR}/IMG_6118.MOV", 5, 10)
text6 = create_text_clip("All dry and fluffy", 2, "top")
clips.append(CompositeVideoClip([clip6, text6]))

# Scene 7: Final shot - cute chicks
print("Scene 7: Final moments")
...
# Add audio
print("Adding ukulele audio...")
audio = AudioFileClip(f"{INPUT_DIR}/happy-ukulele.mp3")
